...
def main():
    reddit = praw.Reddit(client_id=,
                     client_secret=,
                     password=,
                     user_agent='raspbian:com.WellTimedGimli:V0.1 (by I_Mod_Things)',
                     username='_Well_Timed_Gimli_')

    with open('replied_ids.txt', 'w+') as replied:
        replied_ids = deque(replied, maxlen=20)
        replied.close()
...
def getPushshiftData(reddit,replied_ids):
    url = 'https://api.pushshift.io/reddit/search/comment/?q=%22and%20my%20bow%22&locked=false%22&collapsed_because_crowd_control=false%22&size=5'
    #print(url)
